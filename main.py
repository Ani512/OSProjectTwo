...
def tellerRun(teller, tellerQueue):
    ...
    while True:
        try:
            ...
            printUsingLock(f"{customer} asks for a {customer.getTransactionType()} transaction")
            printUsingLock(f"{currentTeller} is handling the {customer.getTransactionType()} transaction")
            ...
            if customer.transactionType == 'withdraw':
                ...
                printUsingLock(f"{currentTeller} is getting the manager's permission")
                # the teller thread should block for a random duration from 5 to 30 ms
                time.sleep(random.uniform(0.05, 0.3))
                printUsingLock(f"{currentTeller} has got the manager's permission")
            ...
            printUsingLock(f"{currentTeller} finishes {customer}'s {customer.getTransactionType()} transaction")
...
# creating threads for the tellers
    ...
    for teller in range(0, len(tellers)):
        tellerThreadList.append(Thread(name="tellerThread"+str(teller), target=tellerRun, args=(tellers[teller], tellerQueue)))
...
# creating threads for the 50 customers 
    ...
    for customer in range(0, len(customers)):
        customerThreadList.append(Thread(name="customerThread"+str(customer), target=customerRun, args=(customers[customer], tellerQueue)))
